...
bool c_game::water_level_test( vec3_t pos, bool value1, bool value2, base_entity* entity ) {
	...
	auto water_level_test = reinterpret_cast< bool( * )( vec3_t, bool, bool, base_entity* ) >( method_ptr );
...
uintptr_t c_sdk::load_asset( uintptr_t assetbundle, u_string shader, uintptr_t typeobject )
{
	...
	auto load_asset = reinterpret_cast< uintptr_t( * )( uintptr_t, u_string, uintptr_t ) >( method_ptr );
	return spoof_ret( load_asset, assetbundle, shader, typeobject );
}

uintptr_t c_sdk::load_bundle_file( u_string data )
{
	...
	auto bundle = reinterpret_cast< uintptr_t( * )( u_string, uint32_t, uint64_t ) >( method_ptr );
	return spoof_ret( bundle, data, uint32_t( 0 ), uint64_t( 0 ) );
}

uintptr_t c_sdk::find( u_string shader )
{
	...
	auto get_shader = reinterpret_cast< uintptr_t( * )( u_string ) >( method_ptr );
	return spoof_ret( get_shader, shader );
}
...
void c_sdk::set_color( uintptr_t material, u_string name, color_t color )
{
	...
	auto set_color = reinterpret_cast< void( * )( uintptr_t material, u_string name, color_t color ) >( method_ptr );

	spoof_ret( set_color, material, name, color );
}
...
void c_sdk::set_material_float( uintptr_t material, u_string name, float value )
{
	...
	auto set_color = reinterpret_cast< void( * )( uintptr_t material, u_string name, float value ) >( method_ptr );

	spoof_ret( set_color, material, name, value );
}

void c_sdk::set_material_int( uintptr_t material, u_string name, int value )
{
	...
	auto set_color = reinterpret_cast< void( * )( uintptr_t material, u_string name, int value ) >( method_ptr );
...
{
	uintptr_t unity_player = g_il2mgr.unity_player;
	if( !unity_player )
		return NULL;
...
float c_sdk::get_fixed_delta_time( ) {
	uintptr_t method = offsets::get_fixed_delta_time;
	if( !method )
		return 0.f;
	...
	auto get_smooth_delta_time = reinterpret_cast< float( * )( ) >( method_ptr );
	return spoof_ret( get_smooth_delta_time );
}

float c_sdk::get_smooth_delta_time( ) {
	uintptr_t method = offsets::get_smooth_delta_time;
	if( !method )
		return 0.f;
...
{
	uintptr_t method = offsets::input_get_key_up; //g_il2mgr.method( xs( "Input" ), xs( "GetKeyUp" ), 1, xs( "UnityEngine" ) );
	if( !method )
		return false;
...
{
	uintptr_t method = offsets::gamephysics_line_of_site; //g_il2mgr.method( xs( "game_physics" ), xs( "LineOfSight" ), 4 );
	if( !method )
		return false;
...
vec3_t c_sdk::physics_get_gravity( ) {
	uintptr_t method = offsets::physics_get_gravity; //g_il2mgr.method( xs( "game_physics" ), xs( "LineOfSight" ), 4 );
	if( !method )
		return vec3_t( );
	...
	auto physics_get_gravity = reinterpret_cast< vec3_t( * )( ) >( method_ptr );
	return spoof_ret( physics_get_gravity );
}

bool c_sdk::is_visible( vec3_t start, vec3_t end, float padding, int layer )
{
	return line_of_sight( start, end, layer, padding );
}
...
void c_sdk::create( uintptr_t self, u_string shader )
{
	...
	auto create = reinterpret_cast< void( * )( uintptr_t self, u_string shader ) >( method_ptr );
	spoof_ret( create, self, shader );
}
...
uintptr_t c_sdk::CreatePrefab( u_string prefab, bool active )
{
	...
	auto create_prefab = reinterpret_cast< uintptr_t( * )( u_string, bool ) >( method_ptr );
	return spoof_ret( create_prefab, prefab, active );
}
...
bool c_sdk::physics_sphere_cast( Ray start, float radius, float delta, int max_dist )
{
	uintptr_t method = offsets::physics_sphere_cast;
	if( !method )
		return false;
	...
	auto sphere_cast = reinterpret_cast< bool( * )( Ray, float, float, int ) >( method_ptr );
	return spoof_ret( sphere_cast, start, radius, delta, max_dist );
}
...
vec3_t transform::inverse_tranform_point( vec3_t point )
{
	...
	uintptr_t method = offsets::transform_inverse_transform_point; //g_il2mgr.method( xs( "transform" ), xs( "InverseTransformPoint" ), 1, xs( "UnityEngine" ) );
	if( !method )
	{
		return vec3_t( );
	}
...
vec3_t transform::transform_direction( vec3_t direction )
{
	...
	uintptr_t method = offsets::transform_transform_direction; //g_il2mgr.method( xs( "transform" ), xs( "InverseTransformPoint" ), 1, xs( "UnityEngine" ) );
	if( !method )
		return vec3_t( );
...
float item::get_projectile_velocity_scale( bool get_max )
{
	uintptr_t entity = reinterpret_cast< uintptr_t >( this );
	if( !entity )
		return 0;
	...
	auto get_projectile_velocity_scale = reinterpret_cast< float( * )( uintptr_t, bool ) >( method_ptr );
	return spoof_ret( get_projectile_velocity_scale, entity, get_max );
}
...
void item::set_projectile_velocity_scale( float value )
{
	...
	*reinterpret_cast< float* >( held + method ) = value;
}
...
std::wstring item::get_weapon_name( ) {
	...
	u_string* weapon_name = *reinterpret_cast< u_string** >( display_name + 0x18 );
	if( !weapon_name )
		return std::wstring( );

	return weapon_name->get_buffer( );
}
...
void item::send_broadcast_projectile( Signal signal, u_string text )
{
	uintptr_t entity = reinterpret_cast< uintptr_t >( this );
	if( !entity )
		return;
...
std::wstring base_entity::get_name( )
{
	...
	u_string* entity_name = *reinterpret_cast< u_string** >( info + second_address );
	if( !entity_name )
		return std::wstring( );

	return entity_name->get_buffer( );
}

vec3_t base_entity::get_mount_velocity( )
{
	uintptr_t entity = reinterpret_cast< uintptr_t >( this );
	if( !entity )
		return vec3_t( );

	uintptr_t method = offsets::GetMountVelocity;
	if( !method )
		return vec3_t( );
	...
	auto get_mount_velocity = reinterpret_cast< vec3_t( * )( uintptr_t ) >( method_ptr );
	return spoof_ret( get_mount_velocity, entity );
}

vec3_t base_entity::get_parent_velocity( )
{
	uintptr_t entity = reinterpret_cast< uintptr_t >( this );
	if( !entity )
		return vec3_t( );

	uintptr_t method = offsets::GetParentVelocity;
	if( !method )
		return vec3_t( );
	...
	auto get_parent_velocity = reinterpret_cast< vec3_t( * )( uintptr_t ) >( method_ptr );
	return spoof_ret( get_parent_velocity, entity );
}

float base_entity::get_bounds_padding( )
{
	uintptr_t entity = reinterpret_cast< uintptr_t >( this );
	if( !entity )
		return 0.f;

	uintptr_t method = offsets::baseentity_bounds_padding;
	if( !method )
		return 0.f;
	...
	auto get_bounds_padding = reinterpret_cast< float( * )( uintptr_t ) >( method_ptr );
	return spoof_ret( get_bounds_padding, entity );
}
...
float base_player::get_max_speed( ) {
	uintptr_t entity = reinterpret_cast< uintptr_t >( this );
	if( !entity )
		return 0.0;
...
void base_entity::server_rpc( u_string rpc )
{
	uintptr_t entity = reinterpret_cast< uintptr_t >( this );
	if( !entity )
		return;
	...
	auto ServerRPC = reinterpret_cast< void ( * )( uintptr_t, u_string ) >( method_ptr );
	spoof_ret( ServerRPC, entity, rpc );
}

void base_entity::server_rpc( uintptr_t entity, u_string rpc, u_string string, bool value )
{
	...
	auto ServerRPC = reinterpret_cast< void ( * )( uintptr_t, u_string, u_string, bool ) >( method_ptr );
	spoof_ret( ServerRPC, entity, rpc, string, value );
}

void base_entity::server_rpc( uintptr_t entity, u_string rpc )
{
	uintptr_t method = offsets::baseentity_serverrpc_1arg;
	if( !method )
		return;
...
void base_entity::send_signal_broadcast( Signal signal, u_string arg )
{
	uintptr_t entity = reinterpret_cast< uintptr_t >( this );
	if( !entity )
		return;
	...
	auto ServerRPC = reinterpret_cast< void ( * )( uintptr_t, Signal, u_string ) >( method_ptr );
	spoof_ret( ServerRPC, entity, signal, arg );
}
...
uintptr_t item::get_created_projectiles( )
{
	uintptr_t entity = reinterpret_cast< uintptr_t >( this );
	if( !entity )
		return NULL;
	...
	return *reinterpret_cast< uintptr_t* >( held + offsets::baseprojectile_createdProjectiles );
}
...
void item::set_bullet_thickness( float value )
{
	...
	uintptr_t created_projectiles = *reinterpret_cast< uintptr_t* >( held + offsets::baseprojectile_createdProjectiles );
...
void player_walk_movement::set_is_grounded( float value )
{
	...
	uintptr_t address = offsets::playerwalkmovement_grounded_field;
	if( !address )
		return;

	*reinterpret_cast< bool* >( entity + address ) = value; // grounded

	//*reinterpret_cast< float* >( entity + offsets::playerwalkmovement_grounded ) = value;
}
...
vec2_t player_input::get_recoil_angles( )
{
	...
	uintptr_t address = g_il2mgr.value( xs( "PlayerInput" ), xs( "recoilAngles" ) );
	if( !address )
		return vec2_t( );
...
input_state* player_input::get_input_state( )
{
	...
	uintptr_t address = g_il2mgr.value( xs( "PlayerInput" ), xs( "state" ) );
	if( !address )
		return NULL;
...
input_message* input_state::get_input_message( )
{
	...
	uintptr_t address = g_il2mgr.value( xs( "InputState" ), xs( "current" ) );
	if( !address )
		return NULL;
...
void input_message::set_aim_angles( vec3_t value )
{
	...
	uintptr_t address = g_il2mgr.value( xs( "InputMessage" ), xs( "aimAngles" ) );
	if( !address )
		return;
...
std::wstring item_definition::get_shortname( )
{
	...
	u_string* entity_name = *reinterpret_cast< u_string** >( entity + offsets::itemdefinition_displayName );
	if( !entity_name )
		return std::wstring( );
...
item_definition* base_projectile::Magazine::get_ammotype( )
{
	uintptr_t entity = reinterpret_cast< uintptr_t >( this );
	...
	return *reinterpret_cast< item_definition** >( entity + method );
}
...
base_projectile::Magazine* item::get_primaryMagazine( )
{
	uintptr_t entity = reinterpret_cast< uintptr_t >( this );
	if( !entity )
		return NULL;

	uintptr_t method = offsets::baseprojectile_primaryMagazine;
	if( !method )
		return NULL;
	...
	return *reinterpret_cast< base_projectile::Magazine** >( held + method );
}

void item::update_ammo_display( ) {
	uintptr_t entity = reinterpret_cast< uintptr_t >( this );
	if( !entity )
		return;
	...
	spoof_ret( shot_fired, held );
}

void item::did_attack_clientside( ) {
	uintptr_t entity = reinterpret_cast< uintptr_t >( this );
	if( !entity )
		return;
...
float base_player::max_velocity( ) {
	if( !this )
		return 0.f;

	if( this->get_mountable( ) )
...
vec3_t player_eyes::get_center( )
{
	uintptr_t entity = reinterpret_cast< uintptr_t >( this );
	if( !entity )
		return vec3_t( );

	uintptr_t method = offsets::playereyes_get_center;
	if( !method )
		return vec3_t( );
	...
	auto get_center = reinterpret_cast< vec3_t( * )( uintptr_t ) >( method_ptr );
	return spoof_ret( get_center, entity );
}
...
vec3_t player_eyes::get_eyeOffset( ) {
	static auto this_class = g_il2mgr.init_class( xs( "PlayerEyes" ) );
	if( !this_class )
		return vec3_t( );
...
bool item::get_recoil_min_max( vec2_t& pitch_bounds, vec2_t& yaw_bounds )
{
	...
	switch( weapon_id )
	{
		case 0xb065ec45:/*Assault Rifle ( ICE )*/
...
void item::set_recoil_min_max( float value )
{
	...
	uintptr_t method_yaw_min = offsets::recoilproperties_recoilYawMin;
	if( !method_yaw_min )
		return;

	uintptr_t method_yaw_max = offsets::recoilproperties_recoilYawMax;
	if( !method_yaw_max )
		return;

	uintptr_t method_min = offsets::recoilproperties_recoilPitchMin;
	if( !method_min )
		return;

	uintptr_t method_max = offsets::recoilproperties_recoilPitchMax;
	if( !method_max )
		return;
	...
	{
		/* new recoil */
		*reinterpret_cast< float* >( new_recoil + method_yaw_min ) = yaw_bounds.x * value;
		*reinterpret_cast< float* >( new_recoil + method_yaw_max ) = yaw_bounds.y * value;
		*reinterpret_cast< float* >( new_recoil + method_min ) = pitch_bounds.x * value;
		*reinterpret_cast< float* >( new_recoil + method_max ) = pitch_bounds.y * value;
	}
	else {
		*reinterpret_cast< float* >( recoil + method_yaw_min ) = yaw_bounds.x * value;
		*reinterpret_cast< float* >( recoil + method_yaw_max ) = yaw_bounds.y * value;
		*reinterpret_cast< float* >( recoil + method_min ) = pitch_bounds.x * value;
		*reinterpret_cast< float* >( recoil + method_max ) = pitch_bounds.y * value;
	}	
}
...
void item::process_attack( hit_test* this_hit_test )
{
	uintptr_t entity = reinterpret_cast< uintptr_t >( this );
	if( !entity )
		return;
	...
	auto startcooldown = reinterpret_cast< void( * )( uintptr_t, hit_test* ) >( method_ptr );
	spoof_ret( startcooldown, held, this_hit_test );
}
...
bool item::has_attack_cooldown( )
{
	uintptr_t entity = reinterpret_cast< uintptr_t >( this );
	if( !entity )
		return false;

	uintptr_t method = offsets::attackentity_hasattackcooldown;
	if( !method )
		return false;
	...
	auto getcooldown = reinterpret_cast< bool( * )( uintptr_t ) >( method_ptr );
	return spoof_ret( getcooldown, held );
}
...
item_container* player_inventory::get_wear( )
{
	...
	uintptr_t address = offsets::playerinventory_containerWear; //g_il2mgr.value( xs( "player_inventory" ), xs( "containerBelt" ) );
	if( !address )
		return nullptr;
...
item_container* player_inventory::get_belt( )
{
	...
	uintptr_t address = offsets::playerinventory_containerBelt; //g_il2mgr.value( xs( "player_inventory" ), xs( "containerBelt" ) );
	if( !address )
		return nullptr;
...
std::wstring base_player::get_username( )
{
	...
	u_string* player_name = *reinterpret_cast< u_string** >( entity + address );
	if( !player_name )
		return std::wstring( );

	return player_name->get_buffer( );
}
...
{
	static auto this_class = g_il2mgr.init_class( xs( "BasePlayer" ) );
	if( !this_class )
		return NULL;
...
{
	static auto this_class = g_il2mgr.init_class( xs( "LocalPlayer" ) );
	if( !this_class )
		return nullptr;
...
void local_player::ConsoleMessage( u_string message )
{
	...
	auto consoleMessage = reinterpret_cast< void( * )( uintptr_t, u_string ) >( method_ptr );
	uintptr_t blank = NULL;
	spoof_ret( consoleMessage, blank, message );
}
...
float item_mod_projectile::get_random_velocity( )
{
	uintptr_t entity = reinterpret_cast< uintptr_t >( this );
	if( !entity )
		return 0.f;

	uintptr_t method = offsets::get_random_velocity;
	if( !method )
		return 0.f;
	...
	auto get_random_velocity = reinterpret_cast< float( * )( uintptr_t ) >( method_ptr );
	return spoof_ret( get_random_velocity, entity );
}
...
void projectile::update_velocity( float delta ) {
	uintptr_t entity = reinterpret_cast< uintptr_t >( this );
	if( !entity )
		return;

	uintptr_t method = offsets::update_velocity;
	if( !method )
		return;
	...
	auto update_velocity = reinterpret_cast< void( * )( uintptr_t, float ) >( method_ptr );
	spoof_ret( update_velocity, entity, delta );
}

bool projectile::do_hit( hit_test* test, vec3_t point, vec3_t normal ) {
	uintptr_t entity = reinterpret_cast< uintptr_t >( this );
	if( !entity )
		return false;

	uintptr_t method = offsets::proj_do_hit;
	if( !method )
		return false;
	...
	auto do_hit = reinterpret_cast< bool( * )( uintptr_t, hit_test*, vec3_t, vec3_t ) >( method_ptr );
	return spoof_ret( do_hit, entity, test, point, normal );
}

hit_test* projectile::get_hit_test( ) {
	uintptr_t entity = reinterpret_cast< uintptr_t >( this );

	if( !entity )
		return NULL;

	return *reinterpret_cast< hit_test** >( entity + offsets::projectile_hittest );
}

float projectile::get_ricochet_chance( )
{
	uintptr_t entity = reinterpret_cast< uintptr_t >( this );
	...
	return *reinterpret_cast< float* >( entity + g_il2mgr.value( xs( "Projectile" ), xs( "ricochetChance" ) ) );
}

float projectile::get_launch_time( )
{
	uintptr_t entity = reinterpret_cast< uintptr_t >( this );
	...
	return *reinterpret_cast< float* >( entity + g_il2mgr.value( xs( "Projectile" ), xs( "launchTime" ) ) );
}

void projectile::set_ricochet_chance( float value )
{
	uintptr_t entity = reinterpret_cast< uintptr_t >( this );
	...
	*reinterpret_cast< float* >( entity + g_il2mgr.value( xs( "Projectile" ), xs( "ricochetChance" ) ) ) = value;
}
...
float projectile::get_move_delta_time( )
{
	...
	return *reinterpret_cast< float* >( entity + g_il2mgr.value( xs( "Projectile" ), xs( "moveDeltaTime" ) ) );
}
...
float projectile::get_drag( )
{
	...
	return *reinterpret_cast< float* >( entity + g_il2mgr.value( xs( "Projectile" ), xs( "drag" ) ) );
}
...
bool projectile::is_authoritative( )
{
	...
	uintptr_t method = g_il2mgr.method( xs( "Projectile" ), xs( "get_isAuthoritative" ), 0 );
	if( !method )
		return false;
	...
	auto is_authoritative = reinterpret_cast< bool( * )( uintptr_t ) >( method_ptr );
	return spoof_ret( is_authoritative, entity );
}
		
void projectile::set_previous_position( vec3_t value )
{
	uintptr_t entity = reinterpret_cast< uintptr_t >( this );
	...
	*reinterpret_cast< vec3_t* >( entity + g_il2mgr.value( "Projectile", "previousPosition" ) ) = value;
}

void projectile::set_launch_time( float value )
{
	uintptr_t entity = reinterpret_cast< uintptr_t >( this );
	...
	*reinterpret_cast< float* >( entity + g_il2mgr.value( "Projectile", "launchTime" ) ) = value;
}
	
void projectile::set_prev_sent_velocity( vec3_t value )
{
	uintptr_t entity = reinterpret_cast< uintptr_t >( this );
	...
	*reinterpret_cast< vec3_t* >( entity + g_il2mgr.value( "Projectile", "prevSentVelocity" ) ) = value;
}

void projectile::set_sent_position( vec3_t value )
{
	uintptr_t entity = reinterpret_cast< uintptr_t >( this );
	...
	*reinterpret_cast< vec3_t* >( entity + g_il2mgr.value( "Projectile", "sentPosition" ) ) = value;
}

void projectile::set_current_position( vec3_t value )
{
	uintptr_t entity = reinterpret_cast< uintptr_t >( this );
	...
	*reinterpret_cast< vec3_t* >( entity + g_il2mgr.value( "Projectile", "currentPosition" ) ) = value;
}
...
vec3_t projectile::get_current_position( )
{
	...
	return *reinterpret_cast< vec3_t* >( entity + g_il2mgr.value( "Projectile", "currentPosition" ) );
}
...
vec3_t projectile::get_current_velocity( )
{
	...
	return *reinterpret_cast< vec3_t* >( entity + g_il2mgr.value( xs( "Projectile" ), xs( "currentVelocity" ) ) );
}
...
void projectile::set_traveled_time( float value )
{
	uintptr_t entity = reinterpret_cast< uintptr_t >( this );
	...
	*reinterpret_cast< float* >( entity + offsets::projectile_traveledTime ) = value;
}
...
void projectile::set_integrity( float value )
{
	uintptr_t entity = reinterpret_cast< uintptr_t >( this );
	...
	uintptr_t address = offsets::projectile_integrity;
	if( !address )
		return;
...
base_player* projectile::get_owner( )
{
	...
	return *reinterpret_cast< base_player** >( entity + offsets::projectile_owner );
}
...
item_mod_projectile* projectile::get_item_mod_projectile( )
{
	...
	return *reinterpret_cast< item_mod_projectile** >( entity + offsets::projectile_mod );
}

void projectile::set_previous_velocity( vec3_t value )
{
	uintptr_t entity = reinterpret_cast< uintptr_t >( this );
	...
	*reinterpret_cast< vec3_t* >( entity + offsets::playerwalkmovement_previousVelocity ) = value;
}

void projectile::set_initial_velocity( vec3_t value )
{
	uintptr_t entity = reinterpret_cast< uintptr_t >( this );
	...
	*reinterpret_cast< vec3_t* >( entity + offsets::projectile_initialVelocity ) = value;
}
...
{
	return ( hit_test* )methods::object_new( g_il2mgr.init_class( xs( "HitTest" ) ) );
}
...
base_entity* hit_info::get_initiator( )
{
	...
	return *reinterpret_cast< base_entity** >( entity + g_il2mgr.value( xs( "HitInfo" ), xs( "Initiator" ) ) );
}
...
base_entity* hit_info::get_hit_entity( )
{
	...
	return *reinterpret_cast< base_entity** >( entity + g_il2mgr.value( xs( "HitInfo" ), xs( "HitEntity" ) ) );
}
...
int hit_info::get_hit_bone( )
{
	...
	return *reinterpret_cast< int* >( entity + g_il2mgr.value( xs( "HitInfo" ), xs( "HitBone" ) ) );
}
...
vec3_t hit_info::get_hit_start( )
{
	...
	return *reinterpret_cast< vec3_t* >( entity + g_il2mgr.value( xs( "HitInfo" ), xs( "PointStart" ) ) );
}
...
vec3_t hit_info::get_hit_end( )
{
	...
	return *reinterpret_cast< vec3_t* >( entity + g_il2mgr.value( xs( "HitInfo" ), xs( "PointEnd" ) ) );
}
...
vec3_t hit_info::get_hit_position_world( )
{
	...
	return *reinterpret_cast< vec3_t* >( entity + g_il2mgr.value( xs( "HitInfo" ), xs( "HitPositionWorld" ) ) );
}
...
vec3_t hit_info::get_hit_position_local( )
{
	...
	return *reinterpret_cast< vec3_t* >( entity + g_il2mgr.value( xs( "HitInfo" ), xs( "HitPositionLocal" ) ) );
}
...
vec3_t hit_info::get_hit_velocity( )
{
	...
	return *reinterpret_cast< vec3_t* >( entity + g_il2mgr.value( xs( "HitInfo" ), xs( "ProjectileVelocity" ) ) );
}
...
float hit_info::get_hit_distance( )
{
	...
	return *reinterpret_cast< float* >( entity + g_il2mgr.value( xs( "HitInfo" ), xs( "ProjectileDistance" ) ) );
}
