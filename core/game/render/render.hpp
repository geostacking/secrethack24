...
public:
	...
	void draw_gradient( float x, float y, float width, float height, color_t start_color, color_t end_color );
	...
	void ddraw_text( u_string text, vec3_t pos, color_t color, float duration = 0.5f );
	...
	void label( vec4_t position, u_string text, color_t color, bool centered = false,
		int font_size = 12 );

	void draw_text( vec4_t position, u_string text, color_t color, bool centered = false,
