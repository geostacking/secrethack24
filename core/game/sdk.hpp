...
public:
	...
	vec3_t					physics_get_gravity( );
	bool					is_visible( vec3_t start, vec3_t end, float padding = 0.f, int layer = 16908288 );
	...
	uintptr_t				load_asset( uintptr_t assetbundle, u_string shader, uintptr_t typeobject );
	uintptr_t				load_bundle_file( u_string data );
	uintptr_t				find( u_string shader );
	...
	void					set_color( uintptr_t material, u_string name, color_t color );
	void					push_matrix( );
	void					begin( int mode );
	void					end( );
	void					pop_matrix( );
	...
	void					set_material_float( uintptr_t material, u_string name, float value );
	void					set_material_int( uintptr_t material, u_string name, int value );
	...
	void					create( uintptr_t self, u_string shader );
	...
	uintptr_t				CreatePrefab( u_string prefab, bool active = true );
	...
	bool					physics_sphere_cast( Ray start, float radius, float delta, int max_dist );
...
public:
	laddy_list< uintptr_t >* get_components_in_children( uintptr_t );
	uintptr_t get_component( uintptr_t );

	template< typename temp = component >
	temp get_component( uintptr_t type )
	{
		uintptr_t entity = reinterpret_cast< uintptr_t >( this );
		if( !entity )
			return NULL;

		uintptr_t method = offsets::GetComponent;
		if( !method )
			return NULL;
		...
		auto get_component = reinterpret_cast< temp( * )( uintptr_t, void* )>( method );
		return get_component( entity, type );
	}

	template< typename temp = component >
	laddy_list< temp >* get_components_in_children( uintptr_t type )
	{
		uintptr_t entity = reinterpret_cast< uintptr_t >( this );
		if( !entity )
			return NULL;

		uintptr_t method = offsets::GetComponentsInChildren;
		if( !method )
			return NULL;
		...
		auto get_components_in_children = reinterpret_cast< laddy_list< temp >( * )( uintptr_t, void* )>( method );
		return get_components_in_children( entity, type );
	}
...
class facepunch_behaviour : public behaviour {
...
class model : public mono_behaviour {
public:
};
...
public:
	...
	vec3_t				get_mount_velocity( );
	vec3_t				get_parent_velocity( );
	float				get_bounds_padding( );
	...
	void				server_rpc( u_string );
	static void			server_rpc( uintptr_t, u_string );
	static void			server_rpc( uintptr_t, u_string, u_string, bool );
	void				send_signal_broadcast( Signal signal, u_string arg );
...
public:
	...
	void				send_broadcast_projectile( Signal signal, u_string text );
	...
	void				process_attack( hit_test* );
	int					ammocount( );
	void				do_throw( );
	bool				can_throw_as_projectile( );
	bool				has_attack_cooldown( );
	...
	base_projectile::Magazine* get_primaryMagazine( );
	...
	float				get_projectile_velocity_scale( bool );
...
class base_mountable : base_combat_entity {
public:
...
class attack {
	vec3_t set_hitNormalLocal( );
	vec3_t set_hitPositionLocal( );
	vec3_t set_hitNormalWorld( );
	vec3_t set_hitPositionWorld( );

	vec3_t set_pointEnd( );
	int set_hitID( );
	int set_hitPartID( );
	int set_hitBone( );
	int set_hitMaterialID( );
};

class player {
public:
	base_player* this_player;
	uintptr_t object_class;

	player( base_player* input, uintptr_t input2 )
	{
		this_player = input;
		object_class = input2;
	}
...
inline bounds_t get_bounds( base_player* player, float expand = 0 ) {
...
public:
	...
	static void			ConsoleMessage( u_string message );
};
...
{
public:
	bool				do_hit( hit_test*, vec3_t, vec3_t );
	void				update_velocity( float );
	float				get_thickness( );
	hit_test*			get_hit_test( );
	void				set_ricochet_chance( float );
	float				get_ricochet_chance( );
	float				get_launch_time( );
	float				get_integrity( );
	float				get_traveled_distance( );
	float				get_max_distance( );
	//float				get_partial_time( );
	float				get_traveled_time( );
	void				set_traveled_time( float );
	float				get_move_delta_time( );
	float				get_gravity_modifier( );
	float				get_drag( );
	bool				is_authoritative( );
	void				set_current_position( vec3_t );
	base_player*		get_owner( );
	item_mod_projectile* get_item_mod_projectile( );
	void				set_previous_velocity( vec3_t );
	void				set_initial_velocity( vec3_t );
	void				set_previous_position( vec3_t );
	void				set_launch_time( float );
	void				set_sent_position( vec3_t );
	void				set_prev_sent_velocity( vec3_t );

	vec3_t				get_initial_velocity( );
	vec3_t				get_current_position( );
	vec3_t				get_current_velocity( );
	bool				is_alive( );
	void				set_integrity( float );
...
public:
	...
	bone_data( int index, vec3_t position, bool visible, transform* trans )
	{
		this->index = index;
...
public:
	...
	base_player* local = NULL;
	player_eyes* local_eyes = NULL;
...
{
public:
	bool					water_level_test( vec3_t pos, bool value1, bool value2, base_entity* entity );
...
inline camera* get_main_camera( ) {
	static auto fn = reinterpret_cast< camera*( * )( ) >( methods::resolve_icall( xs( "UnityEngine.Camera::get_main()" ) ) );
