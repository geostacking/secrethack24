...
void c_framework::create_spacing( int child, int amount ) {
	switch( child )
	{
	case 1:
		cursor1.y += amount;
...
void c_framework::create_tab( std::wstring name, int index, int offset )
{
	...
	g_render.draw_text( vec4_t( position.x + 12, position.y + 33 + ( 45 * index ), 100, 35 ), name.c_str( ), text_color.alpha( alpha, true ), true, 11, dropshadow );
}

void c_framework::create_label( std::wstring text, int child, bool centered )
{
	vec2_t cursor = get_cursor( child );

	g_render.draw_text( vec4_t( cursor.x, cursor.y - 2, 150, 20 ), text.c_str( ), text_color.alpha( alpha, true ), centered, 12, dropshadow );

	create_spacing( child, 22 );
}
	
void c_framework::create_combo_box( std::vector< std::wstring > list, int* selected, int child )
{
	vec2_t cursor = get_cursor( child );
	...
	g_render.draw_text( vec4_t( cursor.x, cursor.y - 3, 150, 20 ), xs( L"<" ),
		left_hovered ? color_t( 255, 245, 245 ) : text_color.alpha( alpha, true ), false, 11, dropshadow );

	g_render.draw_text( vec4_t( cursor.x, cursor.y - 2, 150, 20 ), list[ *selected ].c_str( ),
		text_color.alpha( alpha, true ), true, 11, dropshadow );

	g_render.draw_text( vec4_t( cursor.x + 141, cursor.y - 2, 150, 20 ), xs( L">" ),
...
void c_framework::create_check_box( std::wstring name, bool* value, int child )
{
	vec2_t cursor = get_cursor( child );
	...
	g_render.draw_text( vec4_t( cursor.x + 19, cursor.y - 2, 150, 20 ), name.c_str( ), text_color.alpha( alpha, true ), false, 11 );
...
void c_framework::slider_float( std::wstring name, float* value, float min, float max, int child )
{
	vec2_t cursor = get_cursor( child );
	...
	g_render.draw_filled_rect( vec4_t( cursor.x, cursor.y + 15, max_width, 5 ), background_color.alpha( alpha, true ) );
	g_render.outline_box( vec2_t( cursor.x, cursor.y + 15 ), vec2_t( max_width, 5 ), elements_outline_color.alpha( alpha, true ) );
	...
	*value = std::clamp( *value, min, max );
	...
	std::wstring normal_value = g_sdk.s2ws( g_math.remove_trailing_zeros( hovered ? to_set : *value ) );

	g_render.draw_text( vec4_t( cursor.x, cursor.y - 4, 150, 20 ), name.c_str( ), text_color.alpha( alpha, true ), false, 11, dropshadow );
	g_render.draw_text( vec4_t( cursor.x + max_width - 15, cursor.y - 4, 150, 20 ), 
		normal_value.c_str( ),
		text_color.alpha( alpha, true ), false, 11, dropshadow );

	create_spacing( child, 25 );
}

void c_framework::slider_int( std::wstring name, int* value, int min, int max, int child )
{
	vec2_t cursor = get_cursor( child );
	...
	*value = std::clamp( *value, min, max );
	...
	std::wstring normal_value = std::to_wstring( hovered ? to_set : *value );
...
void c_framework::create_hotkey( std::wstring name, int* key, int child )
{
	elements_count++;
	...
	std::wstring text = name + L": " + L"[" + g_sdk.s2ws( g_aimbot.get_key_from_combo( *key ) ) + L"]";
	...
	g_render.draw_text( vec4_t( pos.x + 3.f, pos.y, 150, 20 ), text.c_str( ), text_color.alpha( alpha, true ), false, 11 );

	create_spacing( child, 23 );
}
...
void c_framework::color_picker( std::wstring name, color_t* color, int child )
{
	++elements_count;
	...
	g_render.draw_text( vec4_t( pos.x + size.x + 8.f, pos.y - 2.f, 150, 20 ), name.c_str( ), text_color.alpha( alpha, true ), false, 11 );
