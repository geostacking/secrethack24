...
void c_menu::draw_menu_tab( ) {
	g_framework.create_check_box( russian_language ? xs( L"русификация" ) : xs( L"rusification" ), &russian_language, 1 );
	g_framework.create_check_box( russian_language ? xs( L"радужный оттенок" ) : xs( L"rainbow accent" ), &options::rainbow_accent, 1 );
	g_framework.color_picker( russian_language ? xs( L"цвет оттенка" ) : xs( L"menu accent color" ), &options::accent_color, 1 );

	g_framework.create_label( russian_language ? xs( L"выбор конфига" ) :  xs( L"quick config" ), 1 );
	g_framework.create_combo_box( cfg_vector, &options::cfg_index, 1 );
	g_framework.create_check_box( russian_language ? xs( L"загрузить" ) : xs( L"load" ), &options::load_cfg, 1 );
	g_framework.create_check_box( russian_language ? xs( L"сохранить" ) : xs( L"save" ), &options::save_cfg, 1 );

	g_framework.create_label( russian_language ? xs( L"есп флаги шрифта" ) : xs( L"esp font flags" ), 1 );
	g_framework.create_combo_box( font_flags, &options::font_flags, 1 );
				
	g_framework.create_label( russian_language ? xs( L"локал индикаторы" ) : xs( L"local indicators" ), 2 );
	g_framework.create_check_box( russian_language ? xs( L"индикаторы" ) : xs( L"indicators" ), &options::visuals::indicators, 2 );
	if( options::visuals::indicators )
		g_framework.create_check_box( russian_language ? xs( L"флай-хак индикатор" ) : xs( L"fly hack indicator" ), &options::visuals::fly_hack_indicator, 2 );

	g_framework.create_check_box( russian_language ? xs( L"хит саунд" ) : xs( L"hit sound" ), &options::aimbot::hit_sound, 2 );
	g_framework.create_combo_box( sound_list, &options::aimbot::sound_type, 2 );
	g_framework.slider_float( russian_language ? xs( L"громкость" ) :  xs( L"volume" ), &options::aimbot::hit_sound_volume, 0.0f, 100.f, 2 );

	g_framework.create_check_box( russian_language ? xs( L"хит трейсеры" ) : xs( L"draw hit bullet tracers" ), &options::visuals::draw_tracers, 2 );
	g_framework.color_picker( russian_language ? xs( L"цвет хит трейсеров" ) : xs( L"hit tracers color" ), &options::visuals::tracers_color, 2 );
	g_framework.create_check_box( russian_language ? xs( L"линии локала" ) : xs( L"draw local trails" ), &options::visuals::local_trails, 2 );
	g_framework.color_picker( russian_language ? xs( L"цвет линий локал плеера" ) : xs( L"trails color" ), &options::visuals::trails_color, 2 );
	g_framework.create_check_box( russian_language ? xs( L"полоса десинхронизации" ) : xs( L"draw desync bar" ), &options::visuals::show_desync_bar, 2 );
	g_framework.color_picker( russian_language ? xs( L"цвет полосы десинхронизации" ) : xs( L"desync bar color" ), &options::visuals::desync_bar_color, 2 );
	g_framework.create_check_box( russian_language ? xs( L"полоса перезарядки" ) : xs( L"draw reload bar" ), &options::visuals::show_reload_bar, 2 );
	g_framework.color_picker( russian_language ? xs( L"цвет полосы перезарядки" ) : xs( L"reload bar color" ), &options::visuals::reload_bar_color, 2 );

	g_framework.create_label( russian_language ? xs( L"менеджер локаций" ) : xs( L"location manager" ), 2 );
	g_framework.create_hotkey( russian_language ? xs( L"удалить позицию" ) : xs( L"remove last position" ), &options::visuals::remove_positions_key, 2 );
	g_framework.create_hotkey( russian_language ? xs( L"сохранить позицию" ) : xs( L"save pos key" ), &options::visuals::save_pos_key, 2 );
	g_framework.create_check_box( russian_language ? xs( L"позиция дома" ) : xs( L"is home position" ), &options::visuals::is_home_pos, 2 );
	g_framework.color_picker( russian_language ? xs( L"цвет позиции" ) : xs( L"position color" ), &options::visuals::pos_color, 2 );
}
