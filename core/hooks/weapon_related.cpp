...
namespace basemelee {
	...
	{
		auto orig_basemelee_process_attack = reinterpret_cast< void( * )( item*, hit_test* ) >( process_attack );
		...
		base_entity* hit_entity = hittest->get_hit_entity( );
		if( !hit_entity )
			return spoof_ret( orig_basemelee_process_attack, base_item, hittest );

		transform* hit_entity_transform = hit_entity->get_transform( );
		...
		{
			base_entity* closest_object = g_esp.get_closest_object_of_class( xs( "OreHotSpot" ), max_dist );
			if( !closest_object )
				return spoof_ret( orig_basemelee_process_attack, base_item, hittest );

			transform* hot_spot_transform = closest_object->get_transform( );
		...
		else if( hit_entity_class == xs( "TreeEntity" ) )
		{
			...
			base_entity* closest_object = g_esp.get_closest_object_of_class( xs( "TreeMarker" ), max_dist );
			...
			{
				transform* transform = closest_object->get_transform( );
				if( !transform )
					return spoof_ret( orig_basemelee_process_attack, base_item, hittest );
		...
		spoof_ret( orig_basemelee_process_attack, base_item, hittest );
	}
}